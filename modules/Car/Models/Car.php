...
class Car extends Bookable
{
    ...
    public function getDepositAmount(){
        ...
        $deposit_amount = setting_item('car_deposit_amount');
        ...
        if (!empty($location_id = $request->query('location_id'))) {
            ...
            if (!empty($location)) {
                $model_car->join('bc_locations', function ($join) use ($location) {
                    $join->on('bc_locations.id', '=', 'bc_cars.location_id')->where('bc_locations._lft', '>=', $location->_lft)->where('bc_locations._rgt', '<=', $location->_rgt);
                });
            }
        }
