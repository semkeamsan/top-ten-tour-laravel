...
class AcledaPaymentGateway extends BaseGateway
{
    public $name = 'Acleda Payment';
    ...
    public function getOptionsConfigs()
    {
        return [
            [
                'type'  => 'checkbox',
                'id'    => 'enable',
                'label' => __('Enable Acleda Payment?')
            ],
            [
                'type'  => 'input',
                'id'    => 'name',
                'label' => __('Custom Name'),
                'std'   => __("Acleda Payment"),
                'multi_lang' => "1"
            ],
            [
                'type'  => 'upload',
    ...
    public function handlePurchaseData($data, $booking, &$payment = null)
    {
        ...
        if (!array_key_exists($main_currency, $supported)) {
            if (!$convert_to) {
                throw new Exception(__("Acleda does not support currency: :name", ['name' => $main_currency]));
            }
