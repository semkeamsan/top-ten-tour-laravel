...
class WingPaymentGateway extends BaseGateway
{
    public $name = 'Wing Payment';
    ...
    public function handlePurchaseData($data, $booking, &$payment = null)
    {
        ...
        if (!array_key_exists($main_currency, $supported)) {
            if (!$convert_to) {
                throw new Exception(__("Wing does not support currency: :name", ['name' => $main_currency]));
            }
