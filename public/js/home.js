...
jQuery(function ($) {
    ...
    $('.form-date-search').each(function () {
        ...
        var options = {
            ...
            minDate: today,
            opens: topTenTour.rtl ? 'right':'right',
            locale: {
                format: "YYYY-MM-DD",
                direction: topTenTour.rtl ? 'rtl':'ltr',
        ...
        check_in_out.daterangepicker(options,
            function (start, end, label) {
                check_in_input.val(start.format(topTenTour.date_format));
                check_in_render.html(start.format(topTenTour.date_format));
                check_out_input.val(end.format(topTenTour.date_format));
                check_out_render.html(end.format(topTenTour.date_format));
            });
    ...
    $('.form-date-search-hotel').each(function () {
        ...
        var options = {
            ...
            onlyShowCurrentMonth: true,
            minDate: today,
            opens: topTenTour.rtl ? 'right':'left',
            locale: {
                format: "YYYY-MM-DD",
                direction: topTenTour.rtl ? 'rtl':'ltr',
                firstDay:daterangepickerLocale.first_day_of_week
        ...
        check_in_out.daterangepicker(options).on('apply.daterangepicker',
            function (ev, picker) {
                ...
                check_in_input.val( picker.startDate.format(topTenTour.date_format) );
                check_in_render.html( picker.startDate.format(topTenTour.date_format) );
                check_out_input.val( picker.endDate.format(topTenTour.date_format) );
                check_out_render.html( picker.endDate.format(topTenTour.date_format) );
    ...
    $('.bravo-form-register [type=submit]').on('click',function (e) {
        ...
        $.ajax({
            'url':  topTenTour.routes.register,
            'data': {
                'email': form.find('input[name=email]').val(),
                'password': form.find('input[name=password]').val(),
    ...
    $('.btn-submit-enquiry').on('click',function (e) {
        ...
        $.ajax({
            url:topTenTour.url+'/booking/addEnquiry',
    ...
    $('.review_upload_file').on('change',function () {
        ...
        for(var i = 0 ;i < me.get(0).files.length ; i++) {
            ...
            if(!me.showErr){
                $.ajax({
                    url: topTenTour.url + '/media/private/store',
                    data: d,
                    dataType: 'json',
                    type: 'post',
                    contentType: false,
                    ...
                    success: function (res) {
                        me.val('');
                        if (res.status === 0) {
                            topTenTourApp.showError(res);
                        }
                        if (res.data) {
                            var count = $(".review_upload_photo_list > .col-md-2").length;
                            if(count > 5){
                                topTenTourApp.showError('Maximum upload 6 pictures');
                            }else{
    ...
    $(".smart-search .smart-search-location").each(function () {
        ...
        var options = {
            url: topTenTour.url+'/location/search/searchForSelect2',
    ...
    $(document).on("click",".service-wishlist",function(){
        var $this = $(this);
        $.ajax({
            url:  topTenTour.url+'/user/wishlist',
            data: {
...
jQuery(function($){
    ...
    if(topTenTour.pusher_api_key && topTenTour.pusher_cluster){
        var pusher = new Pusher(topTenTour.pusher_api_key, {
            encrypted: true,
            cluster: topTenTour.pusher_cluster
        });
    }
    ...
    if(topTenTour.isAdmin > 0 && topTenTour.pusher_api_key){
        var channel = pusher.subscribe('admin-channel');
        channel.bind('App\\Events\\PusherNotificationAdminEvent', callback);
    }

    if(topTenTour.currentUser > 0 && topTenTour.pusher_api_key){
        var channelPrivate = pusher.subscribe('user-channel-'+topTenTour.currentUser);
